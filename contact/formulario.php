...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($referer !== $form_url && $referer !== $form_url1 && $referer !== $form_url2 && $referer !== $form_url3) {
        ...
        header("Location: $form_url?success=false_url");
        exit();
    }
    ...
    if (empty($name) || empty($phone) || empty($message) || empty($visto)) {
        // Redireccionar de vuelta al formulario con un mensaje de error
        header("Location: $form_url?success=false_data");
        exit();
    }
    ...
    $to_emails = array('L39cfi@gmail.com', 'brett@upandreadyflight.com', 'pete@upandreadyflight.com', 'gonzalof@hotmail.com');
    ...
    header("Location: $form_url?success=true");
    exit();
}
